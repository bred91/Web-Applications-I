...
function DefaultRoute(props) {
    ...
    useEffect(() => {
        if(!props.dirty && filterId !== false && filterId.startsWith('filter-'))
            props.setSelectedFilter(filterId);
    }, [filterId]);

    return (
        <Row className="vh-100">
            {props.loggedIn ? <>
            <Col md={4} xl={3} bg="light" className="below-nav secondary" id="left-sidebar">
                 <MyAside selected={props.filterId} items={props.filters} /> 
            </Col> 
            <Col md={8} xl={9} className="below-nav">
                <Outlet /> 
            </Col></>
             : <Col className='below-nav align-items-center text-center'><h1>WELCOME</h1></Col> }
        </Row>
    );
}
...
function MainLayout(props) {
    ...
    const filteredFilms = props.filmLista;
